...
const Product = ({ product }: ProductProps) => {
  ...
  return (
    <Card
      key={product.id}
      className="overflow-hidden group transition-all duration-300 hover:shadow-lg bg-white border-yellow-50 rounded-xl"
    >
      {/* Image Section */}
      <div className="relative overflow-hidden">
        ...
        />
        {product.isNew && (
          <Badge className="absolute top-3 right-3 bg-[#D4AF37] z-20 font-light px-3 py-1 rounded-full text-white text-xs">
            New
          </Badge>
        )}
      </div>
